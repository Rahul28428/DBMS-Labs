...
struct BST{ //used to implement Bucket
    int value;
    struct BST *left, *right, *parent;
    struct BST* addNode(int data){
  ...
  void recursive_inorder(struct BST* node){
    if (node==NULL){
        return;
    }
    else{
  ...
  struct BST* minValue(struct BST* node_temp){
    struct BST* current = node_temp;
